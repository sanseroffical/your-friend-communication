...
const Avatar = ({ user, isLocal, onClick }: AvatarProps) => {
  ...
  useEffect(() => {
    if (!themeSongUrl) { themeAudioRef.current = null; return; }
    const a = new Audio(themeSongUrl);
    a.preload = "none";
    a.volume = 0.7;
    themeAudioRef.current = a;
    return () => { a.pause(); themeAudioRef.current = null; };
  }, [themeSongUrl]);
  ...
  return (
    <group ref={groupRef} position={[user.position[0], 0.5 * heightScale, user.position[2]]} scale={[buildScale, heightScale, buildScale]} onClick={handleAvatarClick}
